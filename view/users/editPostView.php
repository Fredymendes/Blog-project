...
<div class="container">
  <form class="content-hero" method="POST" action="index.php?action=updateValid&id=<?= $update->getIdPosts() ?>">
    <label for="title">Titre :</label>
    <input class="content-post" type="text" name="title" value="<?= htmlspecialchars($update->getTitle()) ?>">
    <label for="wording">Châpo :</label>
    <input class="content-post" type="text" name="wording" value="<?= htmlspecialchars($update->getWording()) ?>">
    <label for="content">Contenu :</label>
    <textarea id="post" class="content" name="content" style="resize: none;" placeholder="Il était une fois...">
    <?= htmlspecialchars($update->getContent()) ?></textarea>
